...
approved  ← input (backlog item from Step 1)
  ...
  ├─ YES → add "legal-review-required", remove "approved" → STOP
  │         (pipeline paused — waiting for PO to resolve and re-approve)
  └─ NO  ↓
...
contains unresolved open questions (legal, regulatory, or commercialista review).
...
## ⛔ Step 2 Dispatch Blocked — Legal Review Required

This Epic contains unresolved **Open Questions** that require human decision
before implementation tasks can be created. Dispatching now would produce
tasks that developers cannot safely implement.
...
1. Consult the relevant stakeholders (PO, commercialista, legal).
2. For each question, record the decision taken.
3. In the Epic body, **replace** `## ⚠️ Open Questions` with `## ✅ Legal Decisions`
   and document each resolution.
4. Remove label `legal-review-required` and re-add label `approved`.
5. Re-trigger Step 2.
...
**Unblocking criteria** (both must be true):
- Issue body contains `## ✅ Legal Decisions` (not `## ⚠️ Open Questions`)
- Label `legal-review-required` has been removed and `approved` re-added by PO
