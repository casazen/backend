...
1. Identify the right stakeholder for each question (PO, architect, legal, commercialista, etc.).
2. For each question, record the decision taken.
3. In the Epic body, **replace** `## ⚠️ Open Questions` with `## ✅ Decisions`
   and document each resolution.
4. Remove label `open-questions` and re-add label `approved`.
5. Re-trigger Step 2.
...
- Issue body contains `## ✅ Decisions` (not `## ⚠️ Open Questions`)
- Label `open-questions` has been removed and `approved` re-added by PO
