...
| Admin multi-role (A1-17) | `GET/PUT /api/users/{id}/roles` (CasaZen API, distinct from the Auth0 endpoints of the row above): the admin console reads the user's current Auth0 roles and sets its exact role set among `Admin`, `PropertyOwner`, `LongTermLandlord`, `Supplier` (`Guest`/`Staff`/`PropertyManager` have no app context and are not offered). No new Auth0 scope: reuses `read:roles`, `read:role_members`, `create:role_members`, `delete:role_members` (section 2). Refuses to remove `Admin` from the last active administrator (`last_active_admin`, 422) | `UsersController.GetRoles` / `UpdateRoles`, `UserService.UpdateRolesAsync` |
