...
| Deactivated account | `account_inactive` (PL-03) wins: `InactiveAccountMiddleware` refuses the request before any policy, so a deactivated user is never sent to the onboarding. The gate is an authorization outcome, not a second middleware | `InactiveAccountMiddleware` (runbook `auth0.md`) |
