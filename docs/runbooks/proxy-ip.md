...
| `PublicBookingLookup` | `POST api/public/bookings/lookup`, `GET api/public/bookings/{id}/status` (the checkout polls it up to 15 times), `POST api/public/bookings/{id}/confirm-email` ("pay at the property" link, BK-06) | 30 / min | `RateLimiting__PublicBookingLookup__PermitLimit` |
