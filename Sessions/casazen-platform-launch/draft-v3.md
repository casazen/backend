# CasaZen Platform Launch — Integrated Draft v3 (Round 3)

> **Author**: GTM Strategist (Builder) · **Pattern**: builder-validator · **Round**: 3
> **Status**: PROPOSE — final revision fixing Legal **C6** + 4 non-blocking refinements (Product Architect ✓ APPROVED, Financial ✓ APPROVED on v2)
> **Supersedes**: `draft.md` (Round 1) and `draft-v2.md` (Round 2) — both left untouched. This file is self-contained.
...
## Changes from Round 2 (v3)

> Round 2 reached **APPROVE** from Product Architect and Financial Strategist. Legal & Compliance raised **one blocking item (C6)**; this v3 fixes it and folds in four cheap, non-blocking PA/Financial refinements. Everything else is identical to `draft-v2.md`.

| # | Source | Edit (and where) |
|---|--------|------------------|
| **C6** | Legal (**blocking**) | `spec-ltr-recurring-rent` (Phase 1.5, §C) — added regulatory gate: **Stripe Connect with the LANDLORD as merchant of record; CasaZen never holds or settles tenant rent funds** (mirrors `spec-direct-checkout`, landlord ↔ operator) + **residential-rent IVA treatment on the receipt: generally IVA-exempt (Art. 10 DPR 633/72), €2 imposta di bollo on receipts > €77.47 except where cedolare-secca exemption applies** — marked **[COUNSEL_REQUIRED]** |
| RF1 | PA (non-blocking) | `spec-tenant-boundary` (§C) — added cross-cutting invariant: **every new tenant-scoped table inherits the `OrgId` + plan-entitlement convention** (so the parallel Phase 1.5 rent ledger cannot ship un-scoped) |
| RF2 | PA (non-blocking) | §C.1 — `spec-saas-billing` (platform-account Stripe Billing) and the Connect flows (`spec-direct-checkout`, `spec-ltr-recurring-rent`) share `WebhooksController`/`StripeWebhookHandler`; specs must separate **platform-account vs connected-account** event routing (signature verification + async `StripeWebhookJob`) |
| RF3 | PA (non-blocking) | §C.1 + `spec-tenant-boundary` — recorded **migration sequencing**: land `OrgId` migration first (nullable → backfill one default `Org` per `OwnerId` → NOT NULL + FK); Phase 1.5 migrations **rebase** onto the updated `AppDbContextModelSnapshot.cs` (never hand-merge); new Phase 1.5 tables carry `OrgId` from creation — **resolves §D Q1** |
| RF4 | Financial / Coordinator (non-blocking) | §0.2 — **dropped** the stale "market-analysis file not on disk" caveat; the file is **restored** at `Sessions/market-analysis-2026/AI-short/long-term-platform.md` (§5.6 corroborates the €150–400 ARPU band, F9), replaced with a one-line "primary source confirmed on disk" note |
...
- **Primary source confirmed on disk**: `Sessions/market-analysis-2026/AI-short/long-term-platform.md` is present; its **§5.6 ("Unità economiche e scalabilità") corroborates the €150–400/mo ARPU band (F9)**. Market-size facts (F1–F10) are anchored to it (and relayed via `domain-context.md › market-landscape`). **[FACT]**
...
| `spec-tenant-boundary` | 1 | Introduce `Org`/tenant key + `OrgId` FK on Property/Booking/LeaseContract/Payment + plan entitlement. **Invariant (RF1): every new tenant-scoped table inherits the `OrgId` + plan-entitlement convention** (so the parallel Phase 1.5 rent ledger cannot ship un-scoped). Migration sequencing → §C.1 | context-RBAC, migrations | **Tenant data isolation** (invariant enforced on every new table); GDPR controller/processor delineation |
...
| `spec-ltr-recurring-rent` | 1.5 | Recurring-rent **ledger + Hangfire job** (closes the `MonthlyRent`-has-no-billing gap) | `LeaseContract`, Stripe **Connect**, Hangfire | **(C6) Stripe Connect with the LANDLORD as merchant of record — CasaZen never holds/settles tenant rent funds** (mirrors `spec-direct-checkout`, landlord ↔ operator); **residential rent generally IVA-exempt — Art. 10 DPR 633/72 — with €2 _imposta di bollo_ on receipts > €77.47 except where cedolare-secca exemption applies [COUNSEL_REQUIRED]**; recurring-payment PSD2/SCA + consent; rent receipt/invoice |
...
### C.1 Spec refinements (Round 3 / v3)

Cheap, cross-cutting refinements recorded here so the full specs (written later by the Coordinator) inherit them:

- **(RF1 — tenant invariant, PA)** `spec-tenant-boundary` sets a cross-cutting rule: **every new tenant-scoped table MUST carry `OrgId` and honor plan entitlement**. The Phase 1.5 `spec-ltr-recurring-rent` ledger is explicitly bound by this — it cannot ship un-scoped.
- **(RF2 — Stripe webhook routing, PA)** `spec-saas-billing` uses **platform-account** Stripe Billing, while `spec-direct-checkout` and `spec-ltr-recurring-rent` use **connected-account** Stripe Connect flows. Because all three share `WebhooksController` + `StripeWebhookHandler`, each spec MUST separate **platform-account vs connected-account** event routing — verify the correct signing secret per event source and dispatch via the async `StripeWebhookJob`.
- **(RF3 — migration sequencing, PA; resolves §D Q1)** Land `spec-tenant-boundary`'s `OrgId` migration **first**, in three steps: (1) add `OrgId` **nullable**; (2) **backfill** one default `Org` per existing `OwnerId`; (3) set **NOT NULL + FK**. Phase 1.5 migrations then **rebase onto the updated `AppDbContextModelSnapshot.cs` (never hand-merge the snapshot)**, and every new Phase 1.5 table carries `OrgId` from creation. This removes the Phase 1 ↔ 1.5 merge-order ambiguity.

---

## D. Residual open questions

1. **(Product Architect) — RESOLVED in v3 (§C.1, RF3):** the Phase 1 ↔ 1.5 `OrgId` merge-order is settled — `OrgId` migration lands first (nullable → backfill → NOT NULL + FK); Phase 1.5 migrations rebase onto the updated snapshot and new tables carry `OrgId` from creation. No open action.
...
*End of Round 3 draft. All 13 Round-2 conditions resolved + Legal **C6** fixed + 4 non-blocking PA/Financial refinements folded in. Product Architect ✓ and Financial ✓ on v2 — this v3 targets full consensus.*
