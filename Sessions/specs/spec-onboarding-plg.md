...
- **AC1**: `POST /api/users/onboarding` (extends `spec-role-onboarding`) is enhanced to, in one transaction: assign Auth0 role(s) for the chosen `rentalType`, **provision an `Org`** for the new user if none exists (`spec-tenant-boundary`: create `Org`, set `User.OrgId`, `PlanTier = Starter`), and **persist consents**. Idempotent: a user who already has an `Org` is not given a second one.
