...
public static class CheckoutHolds
{
    ...
    /// <summary>
    /// An expired hold of the public checkout, created before <paramref name="cutoffUtc"/>:
    /// <list type="bullet">
    /// <item>still <see cref="BookingStatus.Pending"/> and from the public checkout (<see cref="BookingSource.Direct"/>):
    /// host bookings (<see cref="BookingSource.Manual"/>), OTA bookings and every other status are never holds;</item>
    /// <item>not a "pay at the property" request (<see cref="PaymentOption.OnSite"/>): under decision D5 it waits for the
    /// host's approval, with its own waiting time (BK-06), never the checkout TTL;</item>
    /// <item>with a Stripe PaymentIntent or SetupIntent: a Pending booking without one is not a checkout hold;</item>
    /// <item>without a payment <see cref="PaymentStatus.Processing"/> or <see cref="PaymentStatus.Completed"/>: Stripe said
    /// the guest has paid or is paying (the expiry records it, the webhook confirms the booking), so the dates stay taken.</item>
    /// </list>
    /// </summary>
    public static Expression<Func<Booking, bool>> IsExpired(DateTime cutoffUtc) =>
    ...
    /// <summary>
    /// A booking that takes its dates: not cancelled and, when <paramref name="expiredHoldCutoffUtc"/> is given, not an
    /// expired checkout hold (<see cref="IsExpired"/>). Without a cutoff every booking that is not cancelled counts: the
    /// final check before an insert stays strict, so the dates of a hold whose guest paid late are never given away.
    /// </summary>
    public static Expression<Func<Booking, bool>> OccupiesDates(DateTime? expiredHoldCutoffUtc)
    {
        Expression<Func<Booking, bool>> notCancelled = b => b.Status != BookingStatus.Cancelled;
        if (expiredHoldCutoffUtc is not { } cutoffUtc)
            return notCancelled;

        var booking = notCancelled.Parameters[0];
        var expired = IsExpired(cutoffUtc);
