...
public class VrboAdapter : IChannelAdapter
{
    ...
    public async Task<Dictionary<DateOnly, bool>> UpdatePricingBatchAsync(string externalPropertyId, Dictionary<DateOnly, decimal> pricesByDate)
    {
        ...
        try
        {
            _logger.LogInformation("Updating VRBO pricing batch for property {PropertyId} with {Count} dates",
            ...
            _logger.LogInformation("VRBO batch pricing update complete: {SuccessCount}/{Total} dates succeeded",
                successCount, pricesByDate.Count);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in VRBO batch pricing update for property {PropertyId}", externalPropertyId);
    ...
    private async Task<bool> UpdatePricingWithRetryAsync(string externalPropertyId, DateTime date, decimal price, int retryCount = 0)
    {
        ...
        try
        {
            ...
            if (retryCount < maxRetries && IsRetryableStatusCode(response.StatusCode))
            {
                ...
                _logger.LogWarning("VRBO pricing update failed with status {StatusCode}. Retrying in {DelayMs}ms (attempt {Attempt}/{Max})",
            ...
            _logger.LogWarning("Failed to update VRBO pricing for date {Date}. Status: {StatusCode}, Error: {Error}",
        ...
        {
            _logger.LogError(ex, "Unexpected error updating VRBO pricing for property {PropertyId}", externalPropertyId);
            return false;
        }
    }
